...
const PENDING_APPROVALS = [
  ...
  { id: "APR-002", type: "H&W Allowance", employee: "Oliver Wright", detail: "Gym membership — ┬ú45/month", submitted: "2026-04-19", urgency: "Normal" },
  ...
  { id: "APR-005", type: "Leave Request", employee: "Amina Diallo", detail: "Cong├⌐ Annuel: May 12–23 (10 days)", submitted: "2026-04-18", urgency: "Normal" },
];
...
const BENEFITS_PACKAGES = {
  ...
  NG: { health: "HMO — Hygeia / Leadway", dental: "Included in HMO", pension: "Pension Fund Admin (PFA) 18%", life: "Group Life 3├ù salary", disability: "Included in GLA", eap: "Workplace Options", extras: ["Optical ₦50,000/yr", "Maternity Cover", "Housing Allowance 40%"] },
  IN: { health: "Group Mediclaim ₹5L family", dental: "Included in Mediclaim", pension: "EPF 12% + Gratuity", life: "EDLI + Group Term 3├ù", disability: "ESI (if applicable)", eap: "Optum EAP India", extras: ["Top-up ₹10L", "Parental Insurance ₹3L", "NPS employer 10%"] },
...
const WORKABLE_REQUISITIONS = [
  ...
  { id: "WK-4495", title: "IT Systems Administrator", dept: "IT & Digital", country: "CA", status: "Offer", candidates: 15, shortlisted: 4, interviews: 3, daysOpen: 45, hiringMgr: "Lars M├╝ller", salary: "CAD 72,000–88,000/yr", url: "https://apply.workable.com/nutrition-intl/j/4495" },
                ...
                {timesheetData.filter(r => countryFilter === "ALL" || r.country === countryFilter).map((r, ri) => {
                  ...
                  return (
                    <tr key={ri} style={{ borderBottom: `1px solid ${B.borderLight}` }}
                      ...
                      <td style={{ padding: "8px 6px", textAlign: "center" }}>
                        {r.ot > 0 ? <Badge color={B.orange} bg={B.warningBg}>{r.ot}h @ {r.lawData.otMultiplier}├ù</Badge> : <span style={{ color: B.textMuted }}>—</span>}
                      </td>
                  ...
                  {countryHolidays.map((h, i) => {
                    ...
                    return (
                      <div key={h.id || i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 10px", borderRadius: 6, background: isUpcoming ? B.warningBg : B.bgHover, marginBottom: 4, opacity: isPast ? 0.55 : 1, border: `1px solid ${isUpcoming ? `${B.warning}20` : "transparent"}` }}>
                        ...
                        <Btn variant="ghost" size="sm" onClick={() => { setHolidayForm({ name: h.name, date: h.date }); setShowHolidayEdit({ country: code, idx: i }); }}>✏∩╕Å</Btn>
        ...
        {selectedWF && (
          <div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 16, justifyContent: "flex-end" }}>
              <Btn variant="secondary" onClick={() => { setSelectedWF(null); openEdit(selectedWF); }}>✏∩╕Å Edit This Workflow</Btn>
              ...
              {emp.employmentHistory.slice(0, 8).map((h, i) => {
                ...
                return (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px", borderRadius: 6, background: B.bgHover, marginBottom: 3, fontSize: 11 }}>
                    ...
                    <Btn variant="ghost" size="sm" onClick={() => { setHistoryForm({ field: h.field, oldValue: h.oldValue, newValue: h.newValue, effectiveDate: h.effectiveDate, reason: h.reason, empId: emp.id, empName: `${emp.first} ${emp.last}`, idx: i }); setShowHistoryEdit(true); }}>✏∩╕Å</Btn>
                  </div>
                );
              })}
...
const AUDIT_LOG = [
  ...
  { id: "AUD-0003", timestamp: "2026-04-23T11:05:22Z", user: "Sarah Chen", role: "Manager", action: "Approved", target: "Leave Request: LR-002", detail: "Lars M├╝ller — Sick Leave 2 days approved", ip: "192.0.2.17", location: "Ottawa, CA", session: "SES-9d2e5f" },
    ...
    <div>
      ...
      <Modal open={!!showConfirm} onClose={() => setShowConfirm(null)} title="Confirm Feature Change" width={450}>
        ...
        <div style={{ padding: 12, borderRadius: 6, background: B.warningBg, border: `1px solid ${B.warning}30`, fontSize: 12, color: B.textPrimary, marginBottom: 16 }}>
          ⚠∩╕Å Feature toggles affect all users globally. Disabling a module will hide it from navigation and prevent access until re-enabled.
        </div>
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
...
// ─── NI BRANDED ICON SYSTEM ─────────────────────────────────────────────────
// Circle-Square motif · NI palette · Pixel-crisp at 18–24px
const NIcon = ({ name, size = 18, color = "currentColor" }) => {
  const s = size, h = s/2, r = s*0.22, sq = s*0.36;
  const sw = Math.max(1.2, s*0.065); // stroke weight scales with size
  const icons = {
    // ── NAVIGATION ──────────────────────────────────────────────────────────
    dashboard: <><rect x={h-sq*.55} y={h-sq*.55} width={sq*.48} height={sq*.48} rx={2} fill={color}/><rect x={h+sq*.07} y={h-sq*.55} width={sq*.48} height={sq*.48} rx={2} fill={color} opacity={.45}/><rect x={h-sq*.55} y={h+sq*.07} width={sq*.48} height={sq*.48} rx={2} fill={color} opacity={.45}/><circle cx={h+sq*.31} cy={h+sq*.31} r={r*.72} fill={color} opacity={.7}/></>,
    people: <><circle cx={h-.5} cy={h*.62} r={r*.85} fill={color}/><path d={`M${s*.14},${s*.9} Q${s*.14},${h+sq*.1} ${h-.5},${h+sq*.1} Q${h+sq*.3-.5},${h+sq*.1} ${h+sq*.3-.5},${s*.9}`} fill={color} opacity={.55}/><circle cx={h+sq*.4} cy={h*.72} r={r*.65} fill={color} opacity={.4}/><path d={`M${h+sq*.1},${s*.88} Q${h+sq*.1},${h+sq*.2} ${h+sq*.75},${h+sq*.2} Q${h+sq*1.1},${h+sq*.2} ${h+sq*1.1},${s*.88}`} fill={color} opacity={.28}/></>,
    orgchart: <><circle cx={h} cy={s*.22} r={r*.75} fill={color}/><line x1={h} y1={s*.34} x2={h} y2={s*.5} stroke={color} strokeWidth={sw} strokeLinecap="round"/><line x1={s*.25} y1={s*.5} x2={s*.75} y2={s*.5} stroke={color} strokeWidth={sw} strokeLinecap="round"/><rect x={s*.1} y={s*.54} width={sq*.38} height={sq*.38} rx={1.5} fill={color} opacity={.6}/><circle cx={h} cy={s*.73} r={r*.58} fill={color} opacity={.6}/><rect x={s*.62} y={s*.54} width={sq*.38} height={sq*.38} rx={1.5} fill={color} opacity={.6}/></>,
    onboarding: <><rect x={s*.22} y={s*.2} width={sq*1.1} height={sq*1.22} rx={2.5} fill={color} opacity={.12}/><rect x={s*.22} y={s*.2} width={sq*1.1} height={sq*1.22} rx={2.5} fill="none" stroke={color} strokeWidth={sw}/><circle cx={s*.38} cy={s*.44} r={r*.52} fill={color}/><line x1={s*.5} y1={s*.4} x2={s*.72} y2={s*.4} stroke={color} strokeWidth={sw} strokeLinecap="round"/><line x1={s*.5} y1={s*.52} x2={s*.68} y2={s*.52} stroke={color} strokeWidth={sw*.8} strokeLinecap="round" opacity={.5}/><path d={`M${s*.31},${s*.7} L${s*.42},${s*.81} L${s*.69},${s*.57}`} fill="none" stroke={color} strokeWidth={sw*1.2} strokeLinecap="round" strokeLinejoin="round"/></>,
    time: <><circle cx={h} cy={h} r={s*.36} fill="none" stroke={color} strokeWidth={sw}/><line x1={h} y1={h} x2={h} y2={s*.24} stroke={color} strokeWidth={sw*1.2} strokeLinecap="round"/><line x1={h} y1={h} x2={s*.66} y2={h+1} stroke={color} strokeWidth={sw} strokeLinecap="round"/><circle cx={h} cy={h} r={1.5} fill={color}/></>,
    approvals: <><rect x={s*.2} y={s*.14} width={sq*1.1} height={sq*1.22} rx={2.5} fill={color} opacity={.1}/><rect x={s*.2} y={s*.14} width={sq*1.1} height={sq*1.22} rx={2.5} fill="none" stroke={color} strokeWidth={sw}/><path d={`M${s*.32},${s*.52} L${s*.44},${s*.63} L${s*.62},${s*.4}`} fill="none" stroke={color} strokeWidth={sw*1.4} strokeLinecap="round" strokeLinejoin="round"/><line x1={s*.3} y1={s*.72} x2={s*.7} y2={s*.72} stroke={color} strokeWidth={sw*.8} strokeLinecap="round" opacity={.4}/><line x1={s*.3} y1={s*.8} x2={s*.58} y2={s*.8} stroke={color} strokeWidth={sw*.8} strokeLinecap="round" opacity={.4}/></>,
    allowances: <><rect x={s*.15} y={s*.3} width={sq*1.15} height={sq*.78} rx={2.5} fill={color} opacity={.12}/><rect x={s*.15} y={s*.3} width={sq*1.15} height={sq*.78} rx={2.5} fill="none" stroke={color} strokeWidth={sw}/><circle cx={h} cy={h+s*.04} r={r*.62} fill="none" stroke={color} strokeWidth={sw}/><line x1={h} y1={s*.37} x2={h} y2={s*.44} stroke={color} strokeWidth={sw} strokeLinecap="round"/><line x1={h} y1={s*.63} x2={h} y2={s*.7} stroke={color} strokeWidth={sw} strokeLinecap="round"/></>,
    comp: <><circle cx={h} cy={h} r={s*.35} fill={color} opacity={.1}/><circle cx={h} cy={h} r={s*.35} fill="none" stroke={color} strokeWidth={sw}/><text x={h} y={h+s*.12} textAnchor="middle" fontSize={s*.38} fontWeight="700" fontFamily="Arial,sans-serif" fill={color}>$</text></>,
    learning: <><rect x={s*.16} y={s*.26} width={sq*.5} height={sq*1.05} rx={1.5} fill={color} opacity={.28}/><rect x={s*.36} y={s*.2} width={sq*.5} height={sq*1.05} rx={1.5} fill={color} opacity={.55}/><rect x={s*.56} y={s*.3} width={sq*.38} height={sq*.82} rx={1.5} fill={color} opacity={.2}/><path d={`M${s*.25},${s*.84} L${s*.45},${s*.74} L${s*.65},${s*.84} L${s*.65},${s*.96} L${s*.45},${s*.86} L${s*.25},${s*.96} Z`} fill={color}/></>,
    surveys: <><rect x={s*.22} y={s*.14} width={sq*1.1} height={sq*1.22} rx={2.5} fill={color} opacity={.1}/><rect x={s*.22} y={s*.14} width={sq*1.1} height={sq*1.22} rx={2.5} fill="none" stroke={color} strokeWidth={sw}/><circle cx={s*.34} cy={s*.38} r={1.8} fill={color}/><line x1={s*.42} y1={s*.38} x2={s*.68} y2={s*.38} stroke={color} strokeWidth={sw} strokeLinecap="round"/><circle cx={s*.34} cy={s*.53} r={1.8} fill={color} opacity={.6}/><line x1={s*.42} y1={s*.53} x2={s*.65} y2={s*.53} stroke={color} strokeWidth={sw*.8} strokeLinecap="round" opacity={.6}/><circle cx={s*.34} cy={s*.68} r={1.8} fill={color} opacity={.4}/><line x1={s*.42} y1={s*.68} x2={s*.6} y2={s*.68} stroke={color} strokeWidth={sw*.7} strokeLinecap="round" opacity={.4}/></>,
    workflows: <><circle cx={s*.27} cy={h} r={r*.72} fill="none" stroke={color} strokeWidth={sw}/><rect x={s*.56} y={h-r*.72} width={r*1.44} height={r*1.44} rx={2} fill="none" stroke={color} strokeWidth={sw}/><line x1={s*.27+r*.72} y1={h} x2={s*.56} y2={h} stroke={color} strokeWidth={sw} strokeLinecap="round"/><polygon points={`${s*.54},${h-2.5} ${s*.59},${h} ${s*.54},${h+2.5}`} fill={color}/></>,
    analytics: <><line x1={s*.2} y1={s*.8} x2={s*.8} y2={s*.8} stroke={color} strokeWidth={sw} strokeLinecap="round"/><line x1={s*.2} y1={s*.8} x2={s*.2} y2={s*.2} stroke={color} strokeWidth={sw} strokeLinecap="round"/><rect x={s*.27} y={s*.55} width={sq*.32} height={sq*.7} rx={1.5} fill={color} opacity={.4}/><rect x={s*.44} y={s*.38} width={sq*.32} height={sq*.87} rx={1.5} fill={color} opacity={.65}/><rect x={s*.61} y={s*.26} width={sq*.32} height={sq*.99} rx={1.5} fill={color}/></>,
    admin: <><circle cx={h} cy={h} r={s*.3} fill="none" stroke={color} strokeWidth={sw*1.1}/>{[0,51.4,102.8,154.2,205.7,257.1,308.6].map(a=><line key={a} x1={h+Math.cos(a*Math.PI/180)*s*.25} y1={h+Math.sin(a*Math.PI/180)*s*.25} x2={h+Math.cos(a*Math.PI/180)*s*.38} y2={h+Math.sin(a*Math.PI/180)*s*.38} stroke={color} strokeWidth={sw*1.5} strokeLinecap="round"/>)}<circle cx={h} cy={h} r={r*.58} fill={color}/></>,
    superuser: <><path d={`M${h},${s*.12} L${s*.78},${s*.3} L${s*.74},${s*.68} Q${s*.74},${s*.88} ${h},${s*.9} Q${s*.26},${s*.88} ${s*.26},${s*.68} L${s*.22},${s*.3} Z`} fill={color} opacity={.15}/><path d={`M${h},${s*.12} L${s*.78},${s*.3} L${s*.74},${s*.68} Q${s*.74},${s*.88} ${h},${s*.9} Q${s*.26},${s*.88} ${s*.26},${s*.68} L${s*.22},${s*.3} Z`} fill="none" stroke={color} strokeWidth={sw}/><path d={`M${s*.38},${s*.55} L${s*.46},${s*.63} L${s*.62},${s*.43}`} fill="none" stroke={color} strokeWidth={sw*1.4} strokeLinecap="round" strokeLinejoin="round"/></>,
    performance: <><polyline points={`${s*.18},${s*.72} ${s*.36},${s*.46} ${s*.52},${s*.56} ${s*.68},${s*.3}`} fill="none" stroke={color} strokeWidth={sw*1.2} strokeLinecap="round" strokeLinejoin="round"/><circle cx={s*.68} cy={s*.3} r={r*.6} fill={color}/><line x1={s*.18} y1={s*.8} x2={s*.82} y2={s*.8} stroke={color} strokeWidth={sw*.8} strokeLinecap="round" opacity={.4}/></>,
    compplan: <><circle cx={h} cy={h} r={s*.35} fill={color} opacity={.08}/><circle cx={h} cy={h} r={s*.35} fill="none" stroke={color} strokeWidth={sw}/><line x1={h} y1={s*.3} x2={h} y2={s*.44} stroke={color} strokeWidth={sw*1.2} strokeLinecap="round"/><line x1={h} y1={s*.56} x2={h} y2={s*.6} stroke={color} strokeWidth={sw*1.2} strokeLinecap="round"/><line x1={s*.35} y1={s*.46} x2={s*.65} y2={s*.46} stroke={color} strokeWidth={sw} strokeLinecap="round" opacity={.5}/></>,
    settings: <><circle cx={h} cy={h} r={s*.28} fill="none" stroke={color} strokeWidth={sw*1.1}/>{[0,45,90,135,180,225,270,315].map(a=><line key={a} x1={h+Math.cos(a*Math.PI/180)*s*.28} y1={h+Math.sin(a*Math.PI/180)*s*.28} x2={h+Math.cos(a*Math.PI/180)*s*.4} y2={h+Math.sin(a*Math.PI/180)*s*.4} stroke={color} strokeWidth={sw*1.5} strokeLinecap="round"/>)}<circle cx={h} cy={h} r={r*.5} fill={color}/></>,
    // ── ACTIONS / UTILITY ────────────────────────────────────────────────────
    upload: <><rect x={s*.25} y={s*.55} width={sq*.95} height={sq*.68} rx={2} fill={color} opacity={.12}/><rect x={s*.25} y={s*.55} width={sq*.95} height={sq*.68} rx={2} fill="none" stroke={color} strokeWidth={sw}/><line x1={h} y1={s*.44} x2={h} y2={s*.18} stroke={color} strokeWidth={sw*1.2} strokeLinecap="round"/><polyline points={`${s*.36},${s*.3} ${h},${s*.16} ${s*.64},${s*.3}`} fill="none" stroke={color} strokeWidth={sw*1.2} strokeLinecap="round" strokeLinejoin="round"/></>,
    download: <><rect x={s*.25} y={s*.55} width={sq*.95} height={sq*.68} rx={2} fill={color} opacity={.12}/><rect x={s*.25} y={s*.55} width={sq*.95} height={sq*.68} rx={2} fill="none" stroke={color} strokeWidth={sw}/><line x1={h} y1={s*.18} x2={h} y2={s*.44} stroke={color} strokeWidth={sw*1.2} strokeLinecap="round"/><polyline points={`${s*.36},${s*.33} ${h},${s*.49} ${s*.64},${s*.33}`} fill="none" stroke={color} strokeWidth={sw*1.2} strokeLinecap="round" strokeLinejoin="round"/></>,
    document: <><rect x={s*.2} y={s*.1} width={sq*1.08} height={sq*1.3} rx={2.5} fill={color} opacity={.1}/><rect x={s*.2} y={s*.1} width={sq*1.08} height={sq*1.3} rx={2.5} fill="none" stroke={color} strokeWidth={sw}/><path d={`M${s*.6},${s*.1} L${s*.6},${s*.3} L${s*.8},${s*.3}`} fill="none" stroke={color} strokeWidth={sw}/><line x1={s*.32} y1={s*.5} x2={s*.68} y2={s*.5} stroke={color} strokeWidth={sw*.9} strokeLinecap="round" opacity={.7}/><line x1={s*.32} y1={s*.62} x2={s*.68} y2={s*.62} stroke={color} strokeWidth={sw*.9} strokeLinecap="round" opacity={.5}/><line x1={s*.32} y1={s*.74} x2={s*.54} y2={s*.74} stroke={color} strokeWidth={sw*.9} strokeLinecap="round" opacity={.3}/></>,
    check: <><circle cx={h} cy={h} r={s*.36} fill={color} opacity={.12}/><circle cx={h} cy={h} r={s*.36} fill="none" stroke={color} strokeWidth={sw}/><path d={`M${s*.32},${s*.52} L${s*.44},${s*.64} L${s*.68},${s*.38}`} fill="none" stroke={color} strokeWidth={sw*1.4} strokeLinecap="round" strokeLinejoin="round"/></>,
    alert: <><path d={`M${h},${s*.12} L${s*.84},${s*.82} Q${s*.84},${s*.9} ${h},${s*.9} Q${s*.16},${s*.9} ${s*.16},${s*.82} Z`} fill={color} opacity={.12}/><path d={`M${h},${s*.12} L${s*.84},${s*.82} Q${s*.84},${s*.9} ${h},${s*.9} Q${s*.16},${s*.9} ${s*.16},${s*.82} Z`} fill="none" stroke={color} strokeWidth={sw}/><line x1={h} y1={s*.38} x2={h} y2={s*.6} stroke={color} strokeWidth={sw*1.2} strokeLinecap="round"/><circle cx={h} cy={s*.73} r={1.5} fill={color}/></>,
    star: <><polygon points={`${h},${s*.14} ${s*.59},${s*.42} ${s*.84},${s*.42} ${s*.64},${s*.6} ${s*.72},${s*.86} ${h},${s*.7} ${s*.28},${s*.86} ${s*.36},${s*.6} ${s*.16},${s*.42} ${s*.41},${s*.42}`} fill={color} opacity={.85}/></>,
    globe: <><circle cx={h} cy={h} r={s*.36} fill="none" stroke={color} strokeWidth={sw}/><ellipse cx={h} cy={h} rx={r*.7} ry={s*.36} fill="none" stroke={color} strokeWidth={sw*.8}/><line x1={s*.14} y1={h} x2={s*.86} y2={h} stroke={color} strokeWidth={sw*.8}/><line x1={h} y1={s*.14} x2={h} y2={s*.86} stroke={color} strokeWidth={sw*.8} opacity={.5}/></>,
    desktop: <><rect x={s*.1} y={s*.18} width={sq*1.56} height={sq*1.0} rx={2} fill="none" stroke={color} strokeWidth={sw}/><rect x={s*.1} y={s*.18} width={sq*1.56} height={sq*1.0} rx={2} fill={color} opacity={.08}/><line x1={s*.38} y1={s*.76} x2={s*.62} y2={s*.76} stroke={color} strokeWidth={sw*.8} strokeLinecap="round"/><rect x={s*.42} y={s*.76} width={sq*.35} height={sq*.3} rx={1} fill={color} opacity={.25}/></>,
    tablet: <><rect x={s*.28} y={s*.1} width={sq*.88} height={sq*1.32} rx={2.5} fill="none" stroke={color} strokeWidth={sw}/><rect x={s*.28} y={s*.1} width={sq*.88} height={sq*1.32} rx={2.5} fill={color} opacity={.08}/><circle cx={h} cy={s*.84} r={1.6} fill={color} opacity={.5}/></>,
    mobile: <><rect x={s*.32} y={s*.08} width={sq*.78} height={sq*1.38} rx={3} fill="none" stroke={color} strokeWidth={sw}/><rect x={s*.32} y={s*.08} width={sq*.78} height={sq*1.38} rx={3} fill={color} opacity={.08}/><circle cx={h} cy={s*.86} r={1.8} fill={color} opacity={.5}/><line x1={s*.42} y1={s*.2} x2={s*.58} y2={s*.2} stroke={color} strokeWidth={sw*.8} strokeLinecap="round" opacity={.4}/></>,
    // ── ROLE ICONS ───────────────────────────────────────────────────────────
    employee: <><circle cx={h} cy={h*.72} r={r*.95} fill={color}/><path d={`M${s*.16},${s*.9} Q${s*.16},${h+r*1.1} ${h},${h+r*1.1} Q${s*.84},${h+r*1.1} ${s*.84},${s*.9}`} fill={color} opacity={.55}/><line x1={s*.3} y1={s*.84} x2={s*.7} y2={s*.84} stroke={color} strokeWidth={sw*.7} strokeLinecap="round" opacity={.3}/></>,
    manager: <><circle cx={s*.36} cy={h*.68} r={r*.82} fill={color}/><path d={`M${s*.12},${s*.88} Q${s*.12},${h+r} ${s*.36+r*.5},${h+r}`} fill="none" stroke={color} strokeWidth={sw*1.1} strokeLinecap="round"/><circle cx={s*.66} cy={h*.58} r={r*.68} fill={color} opacity={.55}/><path d={`M${s*.42},${s*.82} Q${s*.52},${h+r*1.2} ${s*.88},${h+r*1.2} Q${s*.88+r},{${h+r*1.2}} ${s*.88+r},${s*.88}`} fill="none" stroke={color} strokeWidth={sw*.9} strokeLinecap="round" opacity={.5}/></>,
    hr: <><rect x={s*.2} y={s*.28} width={sq*1.05} height={sq*.88} rx={2} fill={color} opacity={.12}/><rect x={s*.2} y={s*.28} width={sq*1.05} height={sq*.88} rx={2} fill="none" stroke={color} strokeWidth={sw}/><path d={`M${s*.38},${s*.28} L${s*.38},${s*.18} L${s*.62},${s*.18} L${s*.62},${s*.28}`} fill="none" stroke={color} strokeWidth={sw}/><line x1={s*.32} y1={s*.52} x2={s*.68} y2={s*.52} stroke={color} strokeWidth={sw} strokeLinecap="round" opacity={.7}/><line x1={s*.32} y1={s*.63} x2={s*.6} y2={s*.63} stroke={color} strokeWidth={sw*.8} strokeLinecap="round" opacity={.5}/><circle cx={h} cy={s*.52} r={r*.48} fill={color} opacity={.18}/></>,
    lock: <><rect x={s*.26} y={s*.44} width={sq*1.0} height={sq*.84} rx={2.5} fill={color} opacity={.18}/><rect x={s*.26} y={s*.44} width={sq*1.0} height={sq*.84} rx={2.5} fill="none" stroke={color} strokeWidth={sw}/><path d={`M${s*.36},${s*.44} L${s*.36},${s*.34} Q${s*.36},${s*.16} ${h},${s*.16} Q${s*.64},${s*.16} ${s*.64},${s*.34} L${s*.64},${s*.44}`} fill="none" stroke={color} strokeWidth={sw*1.1}/><circle cx={h} cy={s*.65} r={r*.52} fill={color}/><line x1={h} y1={s*.65} x2={h} y2={s*.76} stroke={color} strokeWidth={sw} strokeLinecap="round"/></>,
    // ── NI MISSION ICONS ─────────────────────────────────────────────────────
    leaf: <><path d={`M${h},${s*.88} Q${h},${h} ${s*.24},${s*.32} Q${s*.36},${s*.12} ${s*.8},${s*.14} Q${s*.86},${s*.6} ${h},${s*.88}`} fill={color} opacity={.55}/><path d={`M${h},${s*.88} Q${h},${h} ${s*.24},${s*.32} Q${s*.36},${s*.12} ${s*.8},${s*.14} Q${s*.86},${s*.6} ${h},${s*.88}`} fill="none" stroke={color} strokeWidth={sw*.9}/><line x1={h} y1={s*.88} x2={s*.38} y2={s*.36} stroke={color} strokeWidth={sw*.8} strokeLinecap="round" opacity={.6}/></>,
    heart: <><path d={`M${h},${s*.78} Q${s*.1},${s*.58} ${s*.1},${s*.4} Q${s*.1},${s*.2} ${h},${s*.28} Q${s*.9},${s*.2} ${s*.9},${s*.4} Q${s*.9},${s*.58} ${h},${s*.78} Z`} fill={color} opacity={.8}/></>,
    growth: <><polyline points={`${s*.18},${s*.8} ${s*.34},${s*.56} ${s*.5},${s*.66} ${s*.66},${s*.38} ${s*.82},${s*.22}`} fill="none" stroke={color} strokeWidth={sw*1.2} strokeLinecap="round" strokeLinejoin="round"/><polyline points={`${s*.68},${s*.22} ${s*.82},${s*.22} ${s*.82},${s*.36}`} fill="none" stroke={color} strokeWidth={sw*1.2} strokeLinecap="round" strokeLinejoin="round"/></>,
    globe2: <><circle cx={h} cy={h} r={s*.36} fill={color} opacity={.1}/><circle cx={h} cy={h} r={s*.36} fill="none" stroke={color} strokeWidth={sw}/><path d={`M${h},${s*.14} Q${s*.7},${h} ${h},${s*.86} Q${s*.3},${h} ${h},${s*.14}`} fill={color} opacity={.25}/><line x1={s*.14} y1={h} x2={s*.86} y2={h} stroke={color} strokeWidth={sw*.8}/></>,
    // ── FALLBACK ──────────────────────────────────────────────────────────────
    placeholder: <><circle cx={h} cy={h} r={s*.35} fill="none" stroke={color} strokeWidth={sw} opacity={.4}/><line x1={h} y1={s*.32} x2={h} y2={s*.54} stroke={color} strokeWidth={sw*1.2} strokeLinecap="round" opacity={.6}/><circle cx={h} cy={s*.68} r={1.5} fill={color} opacity={.6}/></>,
  };
        ...
        <div style={{ height: 50, borderBottom: `1px solid ${B.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", background: B.white, flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ display: "flex", gap: 1, background: B.bgHover, borderRadius: 6, padding: 2, border: `1px solid ${B.border}` }}>
              {[{ key: "desktop", icon: "desktop" }, { key: "tablet", icon: "tablet" }, { key: "mobile", icon: "mobile" }].map(v => (
                <button key={v.key} onClick={() => setViewMode(v.key)} style={{ padding: "4px 8px", borderRadius: 4, border: "none", cursor: "pointer", display:"flex", alignItems:"center", background: viewMode === v.key ? B.accent : "transparent" }}>
                  <NIcon name={v.icon} size={15} color={viewMode === v.key ? "#fff" : B.textMuted} />
                </button>
              ))}
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 6, background: B.bgHover, border: `1px solid ${B.border}`, fontSize: 12, color: B.textSecondary }}>
              <NIcon name="globe" size={14} color={B.textSecondary} /> 14 countries
            </div>
          </div>
        </div>
